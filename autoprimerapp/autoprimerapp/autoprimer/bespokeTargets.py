...
def get_surrounding_sequence(variant):
    start = int(variant.start) - 250
    stop = int(variant.stop) + 250
...
def design_from_coord(variant, options):
    ...
    input_sequence = ap.InputSequence(variant.hgvsc, sequence, gene_name=variant.gene, chrom_number=variant.chromosome,
                                      genomic_coords=(int(variant.start)-250, int(variant.stop)+250), strand="+")
    target = ap.TargetRegion(variant.chromosome + ":" + str(variant.start), sequence, int(variant.start)-250, int(variant.stop)+250, 200, input_sequence)# 450/200 = overhang
    print(type(target))
